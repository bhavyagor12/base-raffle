...
contract Raffle50 is Ownable, ReentrancyGuard, VRFConsumerBaseV2Plus {
    enum Phase { Enter, DrawRequested, Drawn }
    ...
    event DrawRequested(bytes32 requestId);
    ...
    function enter(bytes32[] calldata /*proof*/) external inPhase(Phase.Enter) {
    ...
    function requestRandomness(
    ...
    ) external onlyOwner inPhase(Phase.Enter) {
        ...
        vrfRequestId = coordinator.requestRandomWords(
            IVRFCoordinatorV2Plus.RandomWordsRequest({
                ...
                numWords: 1,
                extraArgs: IVRFCoordinatorV2Plus.ExtraArgsV1({ nativePayment: true })
            })
        );
